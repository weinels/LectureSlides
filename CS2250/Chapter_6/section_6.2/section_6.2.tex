...
\end{equation*}}
\visible<3->{Thus, the general solution must be
\begin{equation*}
...
\begin{itemize}[<+- | alert@+>]
\item The \quotetext{speed} along a trajectory in the direction of an eigenvector depends on the magnitude of the associated eigenvalue:
\begin{itemize}
...
\vect{v}=\bvector{\+1,-2}
\end{equation*}}
\visible<3->{Thus, one solution is
\begin{equation*}
\vect{x_1}=e^{4t}\bvector{\+1,-2}
\end{equation*}}
\visible<4->{But, we need two solutions to form a basis of the solution space!}
\onslide<5-8>
...
\end{aligned}
\end{equation*}}
\visible<8->{Which is true if and only if $\vect{v}=\vect{0}$. But this contradicts that $\vect{v}$ is an eigenvector. We will need to try something else.}
\onslide<9-14>
...
\end{aligned}
\end{equation*}}
\visible<12->{Equating coefficients gives the system or equations:
\begin{equation*}
\begin{matrix}[lll]
...
\end{matrix}}
\end{equation*}}
\visible<14->{Since $\vect{v}$ is an eigenvector, we only need to find a solution for $\vect{u}$.}
\onslide<15-20>
...
\end{equation*}}
\visible<20->{Thus, the general solution is
\begin{equation*}
